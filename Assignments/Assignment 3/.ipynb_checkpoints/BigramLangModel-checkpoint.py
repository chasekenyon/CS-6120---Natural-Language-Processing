...
class BigramLanguageModel(nn.Module):
    ...
    def compute_perplexity(self, split='val', input_length=8, batch_size=4):
        ...
        with torch.no_grad():
            for i in range(0, len(data) - input_length, batch_size):
                input_batch = data[i:i + input_length].unsqueeze(0).to(self.device)
                target_batch = data[i + 1:i + input_length + 1].unsqueeze(0).to(self.device)
...
model.fit(train_iters=25000, eval_iters=500, lr=0.001)
...
perplexity = model.compute_perplexity('val')
